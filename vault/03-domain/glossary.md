...
| Stock | Khái niệm cốt lõi về tồn kho. MVP duy trì `system stock quantity` theo internal location; Warehouse total quantity của SKU bằng tổng quantity tại các internal locations. Quantity tại internal location không được âm. Không canonicalize `on-hand`, `available`, `reserved`, `damaged` hoặc `in-transit`. | Semantics ngoài các quantity effects và negative-stock guard đã duyệt vẫn TBD. |
...
| Transfer | Subsequent relocation giữa tracked internal locations trong cùng một Warehouse. Confirmed Transfer có system record, giảm source quantity, tăng cùng quantity tại destination và không đổi Warehouse total. Transfer không được confirm vượt source quantity. Cross-Warehouse Transfer ngoài MVP. | Exception, partial Transfer, retry/cancel và reversal behavior vẫn OPEN/TBD. |
...
| Pick | Lấy quantity từ một hoặc nhiều tracked source internal locations theo Pick request có SKU/requested quantity. Confirmed Pick giảm source quantity; chỉ full quantity mới fully completed; thiếu quantity là `PARTIAL / INSUFFICIENT`. Pick không được confirm vượt tổng quantity tại các selected source locations. | Lifecycle ngoài approved insufficient branch và negative-stock guard vẫn OPEN/TBD. |
| Adjust | Action riêng sau discrepancy/re-check. Warehouse Staff tạo request; Manager approve/reject; reason bắt buộc, evidence optional; approved Adjust cập nhật affected location quantity nhưng không được tạo affected-location quantity âm. | Rejected-case closure vẫn OPEN/TBD. |
